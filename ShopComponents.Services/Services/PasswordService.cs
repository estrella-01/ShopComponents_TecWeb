...
public class PasswordService : IPasswordService
{
    ...
    public string Hash(string password)
    {
        ...
        byte[] key = Rfc2898DeriveBytes.Pbkdf2(password, salt, _options.Iterations,
            HashAlgorithmName.SHA256, _options.KeySize);
    ...
    public bool Check(string hash, string password)
    {
        ...
        if (parts.Length != 3) throw new FormatException("Hash inválido.");
        var iterations = Convert.ToInt32(parts[0]);
        var salt = Convert.FromBase64String(parts[1]);
        var key = Convert.FromBase64String(parts[2]);
        byte[] keyToCheck = Rfc2898DeriveBytes.Pbkdf2(password, salt, iterations,
            HashAlgorithmName.SHA256, _options.KeySize);
